...
void ZSTD_lazy_loadDictioanry(ZSTD_matchState_t* ms, const BYTE* ip)
{
    ...
    for (U32 idx = ms->nextToUpdate; idx < target; idx++) {
        ...
        ms->hashTable[h + 2] = chainTable[idx & chainMask];
        ms->hashTable[h + 3] = chainTable[ms->hashTable[h + 2] & chainMask];  
        ms->hashTable[h + 4] = chainTable[ms->hashTable[h + 3] & chainMask];
        ms->hashTable[h + 5] = chainTable[ms->hashTable[h + 4] & chainMask];
        ms->hashTable[h + 6] = chainTable[ms->hashTable[h + 5] & chainMask];
        ms->hashTable[h + 7] = chainTable[ms->hashTable[h + 6] & chainMask];
        ms->hashTable[h + 8] = chainTable[ms->hashTable[h + 7] & chainMask];
        ms->hashTable[h]++;
    }
    ms->nextToUpdate = target;
}

void ZSTD_lazy_changeChainTable(ZSTD_matchState_t* ms)
{
    for (U32 h = 0; h < (1U << 17); h++) {
        U32 const rh = h << 4;
...
FORCE_INLINE_TEMPLATE void ZSTD_HcFindBestMatch_dictMatchState(
                        size_t* offsetPtr, size_t* ml, ZSTD_matchState_t* ms,
                        U32 matchIndex, U32 const dictLimit, U32 const mls,
                        U32 nbAttempts, const BYTE* const iLimit, 
                        const BYTE* const prefixStart,
                        U32 const current, const BYTE* const ip)
{
    ZSTD_HcFindBestMatch_dictMatchState_open(offsetPtr, ml, ms, matchIndex, dictLimit, 
